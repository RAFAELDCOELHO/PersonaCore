...
- **Dev environment**: Codex as the development environment; GSD workflow for planning.
<!-- GSD:project-end -->
...
> **PRIMARY training = local Apple Silicon (M3 / MPS), fp32, no AMP.** This strengthens the
> fully-on-device / zero-budget / privacy-by-design thesis: the model trains on the author's own
> machine. **Kaggle P100 (Pascal, fp16-AMP) is an OPTIONAL FALLBACK** for longer/faster runs.
> `RuntimeConfig` resolves device priority **CUDA-P100 → MPS → CPU** via `preflight_device`.
> MPS has no fp16-AMP path → fp32 on MPS (same posture as CPU); fp16 AMP + `GradScaler` is a
> CUDA-only memory measure; the bf16-on-Pascal guard is unchanged and still errors.

- **Primary path — local M3/MPS:** train in **fp32** on Apple Silicon via the MPS backend
  (`torch.backends.mps.is_available()`). No mixed precision, no `GradScaler` (neither helps on MPS);
  unified memory, eager/math ops (no `torch.compile`). The checkpoint/resume infra handles local
  multi-session runs (sleep/interrupt) exactly as it handles Kaggle session kills.
- **Fallback path — Kaggle P100:** use Kaggle's pre-installed PyTorch (do not reinstall torch in
  the notebook). For local CPU dev, pin `torch==2.7.*` (the last release line that still ships a
  Pascal-capable CUDA wheel and a clean CPU wheel) — but the GPU build only matters on Kaggle, and
  there you use whatever Kaggle ships.
...
# Core + demo + dev
# requests only if downloading TinyStories at runtime instead of a local copy / Kaggle Dataset attach
...
`torch.compile`** (Apple codegen path is immature; not worth it at this scale). The
checkpoint/resume infra (below) covers local multi-session runs — laptop sleep/interrupt resumes
bit-for-bit, exactly like a Kaggle session kill.
...
**Primary — local Apple Silicon (M3 / MPS) training:**
...
- Keep the corpus + checkpoints on local disk; checkpoint `latest.pt` every K steps. Runs survive sleep/interrupt and resume bit-for-bit across sessions.

**Fallback — Kaggle P100 training:**
...
│   ├── preflight.py        # preflight_device() — resolve CUDA-P100→MPS→CPU; assert Pascal-compatible CUDA on the P100 fallback
...
### Local laptop / Apple Silicon (PRIMARY: M3/MPS or CPU — training, dev, inference, tests)

This is the **primary training environment**: train in **fp32** on Apple Silicon via the MPS
backend (`torch.backends.mps.is_available()`), or on CPU. `RuntimeConfig` resolves
CUDA-P100 → MPS → CPU via `preflight_device`. No AMP / `GradScaler` / `torch.compile` on MPS.
Runs persist the corpus and checkpoints to local disk and resume across laptop sleep/interrupt
exactly like a Kaggle session kill (see Reproducibility discipline below).

A **Python 3.11 virtual environment is MANDATORY**. This dev box runs Python 3.14, which is
**not a supported target** — torch wheels and Kaggle's image target 3.10/3.11, so develop
...
### Kaggle (P100 GPU training) — OPTIONAL FALLBACK

This path is the **optional fallback** for a longer/faster GPU run; the primary path is local
M3/MPS above. The **public GitHub repo is the single source of truth** (D-05) — Kaggle pulls code
by cloning, never by copy-paste or manual upload:
...
wheel dropped Pascal kernels and would silently brick the P100 — `preflight_device()` smoke-tests
the running wheel before any long run on the P100 fallback. Verify with
`torch.cuda.get_device_name(0)` in cell 1.
...
### Data + checkpoint persistence

- **Primary (local M3):** the `uint16` memmap corpus and checkpoints live on **local disk**.
  Multi-session runs resume from `latest.pt` across sleep/interrupt; nothing is wiped between
  sessions, so there is no separate dataset-versioning step.
- **Fallback (Kaggle P100):** checkpoints land in `/kaggle/working` (wiped at session end) and
  are persisted to a **versioned Kaggle Dataset** so they survive the 30h/week + ~9h/session
  limits and resume exactly. The Kaggle-Dataset step applies only to this fallback path.

### Two Kaggle run modes (D-08) — fallback only

- **Headless "Save & Run All / Commit"** — the default for a long pretrain *on the fallback*.
  Runs unattended; checkpoints persist to a versioned Kaggle Dataset (see above).
