...
---
*Updated 2026-09-13 after Phase 26: the guarantee was tested, not just the code. A one-run canary audit (17 gitignored sidecars: adapter-off, the σ=0 control, 15 noised dp_n8 frontier points; 30 h 43 min unattended on the M3) gives every published DP point an empirical ε lower bound. Verdict artifact `results/phase26_canary.json`, committed BY THE OPERATOR (`8652c15`, the only git write in the audit's lifetime) and pinned to the frontier both ways: **15 CONSISTENT / 0 BROKEN / 0 INCONCLUSIVE**, power gate PASSED (the control reproduces 790/1008 and resolves ε_lower = 2.786 ≥ the smallest claim 0.634), reachable claims 4/15 (σ = 24, 32, 50, 80); the other 11 carry the pre-registered D-13 disclosure that the auditor's ceiling sits below their claimed ε, so CONSISTENT there means "not refuted", not "confirmed". Every noised adapter answered 0/8 members and 0/56 non-members, ε_lower = −0.047 at all 15 points — the accountant's claim is nowhere contradicted, and the audit says exactly how much it could see. Verification `passed` 3/3 SC, 24/24 must-haves; CANARY-01/02 validated. Phases 27-28 remain. Prior entry: *Updated 2026-09-09 after Phase 25: the frontier is mapped and it is EMPTY — 0 of 32 DP and 0 of 6 adversarial points clear all three conditions at either capacity, no candidate was promoted, and the pre-registered null is the published result. The DP arm buys its extraction floor by destroying the memory it was protecting (taught recall 0/1008 at every sigma > 0); the adversarial arm is recipe-confounded (it trains with no replay) and says so. Phases 26-28 remain. Original entry: *Last updated: 2026-08-20 at v4.0 kickoff. Milestone v4.0 "Leakage Mitigation and Relearning Validation" scoped: answer the 88.5% leakage finding v3.0 measured and left unanswered, with two training-time mitigation arms swept into a privacy/utility frontier under an existence gate, and a relearning attack as the adversarial proof the mitigation cannot be cheaply reverted. The four v3.0 requirements moved from Active to Validated. Erasure at higher rank and the frozen-tokenizer retrain are explicitly deferred, not dropped.*
