...
### ✅ v4.0 Leakage Mitigation and Relearning Validation (Phases 20-28) — SHIPPED 2026-09-22

*Kept in full rather than collapsed: the Phase 25 sections below carry sentinel-bounded, retract-in-place continuations (`25-07-CONTINUATION-*`) and the superseded sentences they correct, and Phase 28's renderer and prereg guards read this file live — `tests/test_phase25_correction.py`, `tests/test_phase28_prereg.py`. Archive: [milestones/v4.0-ROADMAP.md](milestones/v4.0-ROADMAP.md) · Audit: [milestones/v4.0-MILESTONE-AUDIT.md](milestones/v4.0-MILESTONE-AUDIT.md) · Requirements: [milestones/v4.0-REQUIREMENTS.md](milestones/v4.0-REQUIREMENTS.md).*
...
- [x] **Phase 24: Adversarial Extraction-Aware Training + the Held-Out Attack Family** - The second arm as a data-mixture ratio, with generalization converted from a disclaimer into a measurement (completed 2026-08-30)
- [x] **Phase 25: Frontier Sweep and the Existence-Gate Verdict** - Both arms, both capacities, one plane, judged by importing the Phase 20 constants (completed 2026-09-09)
- [x] **Phase 26: Empirical Privacy Audit (Canary)** - Test the guarantee rather than the code: an empirical ε lower bound against the claimed upper bound (completed 2026-09-13)
- [x] **Phase 27: Relearning Attack** - Admitted by a gate call on measured numbers; recovery ceiling as the binary verdict, cost-to-recovery as the qualifying instrument (completed 2026-09-16)
- [x] **Phase 28: Report, the Published Null, and Milestone Close** (completed 2026-09-22 — verification `passed`, 4/4 SC, 35/35 must-haves) - Publish whichever way the numbers came out, with every number in prose generated from a committed record
...
- [x] 24-03-PLAN.md — ADVT-02's split as TWO separately-named assertions (`family`, `source_family`) plus the dated 24-03 continuation superseding SC2's unsatisfiable key *(the superseded key measured UNSATISFIABLE on BOTH readings — 140 of 140 two-field `(fact_id, seed_index)` keys and 216 of 216 three-field triples overlap pairwise, complete, because the corpus is a full cross product by construction; the continuation landed **48 insertions / 0 deletions** between fresh `24-03-CONTINUATION-*` sentinels with SC2's own sentence byte-identical, and all four guards in `tests/test_phase24_correction.py` were watched RED — including a same-line duplicate sentinel where `grep -c` returns 1 and `str.count` returns 2)*
- [x] 24-04-PLAN.md — D-04's `contains_refusal` beside `contains_value`, and D-11's clean-frame probe populations pinned before any run *(174 insertions / 0 deletions, additive; `normalize`/`contains_value`/`score_question` byte-unchanged and `def normalize` still occurs once; `phase24_adversarial` absent from the module-level imports by AST, both new imports lazy inside the function. `CLEAN_FRAME_PROBE_FAMILY_IDS = ("F1","F2","F6")` builds BOTH populations — 8 locked facts and 56 filler facts, each landing on **112 distinct questions** (the filler side's 784 rows collapse 7:1 because a clean-frame question is slot-determined), budget-matched 112 vs 112, disjoint, and 0 of the 10 published values found across all 224. `{F1,F2,F6}` re-derived independently as `core_taught`'s `source_family` set in the 864-row corpus (160/160/128). Seven probes watched, six RED and one recorded FALSE GREEN — `fact-keyed` occurs twice in the reading rule, so a one-occurrence edit left the guard correctly passing)*

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 24-05-PLAN.md — the corpus-joined episode builder: `core_taught` only, three families, every prompt proved byte-equal to its committed row, plus the fourth `PERSONA_ALLOWLIST` entry *(**336 episodes measured**, 112 per family and family-independent at the episode unit, built read-only off the committed 864-row corpus — `build_corpus` never called, AST-proved: 3 prose mentions, 0 calls. **All 336 prompts byte-equal** to their committed `prompt_ids` under hard list equality, checked inside the builder AND independently in the tests through `build_recall_prompt` directly. A2 is refused, not dropped: the refusal was watched firing on a monkeypatch-widened family tuple and its message is asserted to name the family and all four reasons. `attack_prompt_ids` is the ONE `persona=` call site (AST: exactly 1 call, not `grep -c`, which counts the docstring); its fourth allowlist entry landed in the SAME commit `c10d017`, watched RED without it — `Left contains one more item: ('scripts/phase24_adversarial.py', 'attack_prompt_ids')`. Full suite **1619 passed, 1 skipped**, 0 failed, exactly +6 over the 1613/1 baseline)*
...
- [x] 24-07-PLAN.md — D-05's four-corner band check and the committed ADVT-03 per-arm scored-token record *(**all four corners measured before any sweep point exists**, converting a post-compute build-time `SystemExit` into a 2.2-second CPU test: `adv_n8` 0.358660 → **0.241009**, `adv_n64` 0.390163 → **0.251734**. The n=64 column was MEASURED, never carried across from n=8. The binding corner is `(adv_n8, upper)` — asserted as a measured ordering, not assumed — and it clears `0.15 + MASK_FRACTION_MARGIN` by **0.041009**; the band's ceiling is asserted too, so "only the floor binds" is checked rather than argued. **24-06's open question decided:** `mask_fraction_min = 0.1111` gets NO gate — it is an A3 episode at 18 scored tokens in 162, so the fraction moves with the attack PROMPT's length, and the well-defined per-episode quantity is the scored-token COUNT, which already has a floor (every adversarial episode measures 18–23 against `MIN_REFUSAL_SCORED_TOKENS = 15`, while the CLEAN pool's shortest answer is 11 and has never been gated). `results/phase24_token_budget.json` ships 12 rows of integer counts with denominators and D-07 multiplicity in the same row — which reaches **8.0× at `adv_n64`'s upper extreme**, so n=64 repeats the whole pool eight times where n=8 passes it once. Both refusals were WATCHED firing with no bytes landing, and the emitter was committed BEFORE its artifact so `provenance.git_sha` 5aed70f contains the code that wrote it (21-REVIEW CR-02's defect, checked not trusted). A one-digit hand edit to a committed `scored_tokens` was watched going RED in two independent tests, then restored under a sha256 equality check. **ADVT-02 and ADVT-03 TICKED; ADVT-01 deliberately not** — no adapter has been trained. Full suite **1645 passed, 1 skipped**, 0 failed, exactly +12 over the 1633/1 baseline)*
...
- [x] 25-18-PLAN.md — the CPU-only curve verdict pass and D-11's lazy candidate-triggered promotion, applied byte-identically to its pre-registration

**Wave 12** *(blocked on Wave 11 completion)*

- [x] 25-19-PLAN.md — the write-once assembly of `results/phase25_frontier.json` (D-31, D-36, D-28, D-29) and the figures drawn only from it

**Wave 13** *(blocked on Wave 12 completion)*

- [x] 25-20-PLAN.md — D-13's committed revert executed and verified, D-37/D-40 discharged, §O1's exception closed, and eight requirements ticked against named evidence — blocking human checkpoint
...
- [x] 26-01-PLAN.md — `scripts/phase26_prereg.py` + `tests/test_phase26_prereg.py`: the dated continuation (rule by reference, extension to all 15 noised dp_n8, power gate, D-09 arithmetic, verdict domain, D-40 continuation) frozen by ancestry guard

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 26-02-PLAN.md — `scripts/phase26_canary.py` driver (one instrument IN/OUT, OFF once, sidecars by hash, `--dry-run`/`--emit`) + `artifacts/com.personacore.phase26.canary.plist`

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 26-03-PLAN.md — `tests/test_phase26_canary.py`: structural tests, the live-path wiring proof through `main()`, power-gate forged-pass RED, sibling link both ways

**Wave 4** *(blocked on Wave 3 completion)*

- [x] 26-04-PLAN.md — operational note (first tracked Phase-26 result), ≈ 25 h budget stated, LaunchAgent kickstart, early-run human gate (OFF sidecar, consumer refusal on real records, control 790/1008)

**Wave 5** *(blocked on Wave 4 completion)*

- [x] 26-05-PLAN.md — `--emit` once when 17 sidecars exist (else the dated D-19 named limitation), operator commits `results/phase26_canary.json` by hand, machine put back
...
- [x] 27-01-PLAN.md — Pre-registration: `scripts/phase27_prereg.py` (admission gate, X by call, Z rule, band, rung ladder, pinned baselines, attacker corpus) + `tests/test_phase27_prereg.py` (ancestry guard, 44-verdict route tripwire, cleared 30/4/1)
- [x] 27-02-PLAN.md — `on_draw=None` capture point on `get_batch_memmap_masked`, threaded through `train()` to both loader sites + `tests/test_phase27_on_draw.py` (byte-neutrality, coverage, seed, resume-chain identity)

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 27-03-PLAN.md — CLI driver `scripts/phase27_relearn.py` (`admit` write-once + calibrate / curve / gate / structural-proof legs on `train()` direct with one shared `TrainConfig`, OQ1-B) + structural tests (per-leg refusals, kwargs trace, AST guards)

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 27-04-PLAN.md — D-10 wiring proof: CPU e2e through `main()` on a tiny GPT + real tokenizer, off-disk config diff, offset-stream digests, disjointness, node-id existence, provenance recompute, pyproject byte-identity

**Wave 4** *(blocked on Wave 3 completion)*

- [x] 27-05-PLAN.md — Close-out: `admit` once on the real frontier (expected MOOT), refusals watched on the real record (D-37), operator commits the record by hand (D-15), D-38 ledger edits, full suite + lint gate
...
- [x] 28-01-PLAN.md — D-25 tomllib dependency test + D-26 sha-pin rename; D-32 docstring fixes with tests
- [x] 28-02-PLAN.md — D-31/D-39 stamp and archived-artifact repairs; 25-UAT complete; 24-UAT items 2-3 disposed (ruling checkpoint)

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 28-03-PLAN.md — results/phase28_ledger.json (one row per open item, closed domain, FIXED requires a test) + tests; developer review checkpoint

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 28-04-PLAN.md — scripts/phase28_report.py renderer + two templates (every number a binding; no bare numeral)

**Wave 4** *(blocked on Wave 3 completion)*

- [x] 28-05-PLAN.md — guards: template scan, obligation resolution, constants, digests, confound, lead; D-04/D-05 expectation ancestry test

**Wave 5** *(blocked on Wave 4 completion)*

- [x] 28-06-PLAN.md — publish: install both blocks, byte-identity RED→GREEN, normalized register widened, developer read, publishing commit (frozen)
...
| 25. Frontier Sweep and the Existence-Gate Verdict | v4.0 | 22/22 | Complete (verification `passed` 2026-09-09; cell corrected at the v4.0 close) | 2026-09-09 — 25-20 closed: D-13's revert executed from the committed `PMSET_REVERT` tuple and proved by `prove_reverted()` (`{'sleep': 1, 'disksleep': 10, 'powernap': 1}`), the three SoftwareUpdate flags back to their declared target, five LaunchAgents booted out (`launchctl list | grep personacore` empty), `KeepAlive` false in every committed plist at close, the polymarket keep-awake restored (§7b); D-37's three reservations discharged on disk (44 adapters verified from bytes, n=8-only canaries in the data, audit target `dp_n8_sigma0p000000` by the committed rule); D-40 handed to Phase 28; §O1's `{add, commit}` exception closed as phase-only after 44 unattended commits. Spend: 81.40 h sweep + 15.77 h recall = 97.17 h inside the 87.86–149.45 h envelope; 2 jetsam kills (largest loss 10.17 min), 3 one-stage halts, 8 stall records all `action_taken: none`. Eight requirements ticked by hand against named artifact fields and guard tests, both corrections kept visible behind `SUPERSEDED:` (ZERO TOLERANCE; the gate never reads the control's extraction), FRONT-04 satisfied in the weaker 0-of-32 / 0-of-6-with-6-refused form and says so. Zero `gsd-sdk` mutation handlers called; the phase-complete step is the orchestrator's. Earlier: 25-19 closed: `results/phase25_frontier.json` assembled write-once (`4030d0e`, 22.3 MB, 44 points inline, ordered keys proved at the write), curve_total_epsilon 2387.299119573244 over 30 summands, verdicts 32 FAIL / 6 INCONCLUSIVE / 6 REFUSED / 0 PASS carried verbatim, held-out A2 756/4576 re-derived, figures `6af3fa0`, 30 tests; FRONT-02/FRONT-03 ticked. Earlier: 25-18 closed 2026-09-09: verdict pass over all 44 records, frontier EMPTY (0 of 32 dp / 0 of 6 adversarial PASS, `null-at-both-capacities`, 0 candidates, tail cost 0); condition (b) produced post hoc for 42 points (D-25-18-RECALL, 15.77 h), adv_n64 leg refused by the route (D-25-18-ADV64-REFUSED); 25-20 remains *(row note wiped again by `roadmap.update-plan-progress`, hand-repaired)* |
| 26. Empirical Privacy Audit (Canary) | v4.0 | 5/5 | Complete | 2026-09-13 — verification `passed` (3/3 SC, 24/24 must-haves, 26-VERIFICATION.md). 26-05 closed on Branch A: `--emit` once on 17 sidecars, `results/phase26_canary.json` (165,830 bytes, `d2a71e2d…`) = 15 CONSISTENT / 0 BROKEN / 0 INCONCLUSIVE, reachable claims 4/15 (σ = 24, 32, 50, 80; the other 11 carry the D-13 `could not have failed` disclosure), `auditor_ceiling 2.7858978325772576`, power gate PASSED (control 2.7859 ≥ 0.6340), exclusions 0/56 and 0/8, pinned to the frontier both ways (`frontier_sha256 1f182b40…`, 16 `adapter_sha256`), frontier still at ONE commit. Committed BY THE OPERATOR at `8652c15` — the only git write in the audit's lifetime (§O1 closed). Run: kickstart 2026-09-11T16:23:57Z → done 2026-09-12T23:07:28Z = 30 h 43 min against ≈ 25 h (σ = 50/80 outside the timing band, cause unmeasured). Close: canary + watcher agents booted out, `launchctl list` empty of phase26, pmset untouched; four idle Phase-25 agents found loaded (runs = 0), left and named for the operator. `make test` 2792 passed / 4 skipped / 0 failed. CANARY-01/02 ticked by hand. Zero `gsd-sdk` mutation handlers. |
| 27. Relearning Attack | v4.0 | 5/5 | Complete | 2026-09-16 — verification `human_needed` resolved by two developer rulings (27-HUMAN-UAT.md): 5/5 ROADMAP SC, 28/28 must-haves (27-VERIFICATION.md); the 16 review findings carried to Phase 28 as named limitations, no adv_* admission until v5.0. the admission gate read MOOT on the measured frontier and the milestone ships that finding: `results/phase27_admission.json` records 0 of 44 PASS (tallies PASS 0 / FAIL 32 / INCONCLUSIVE 6 / REFUSED 6), cleared (a) 30 / (b) 4 / (c) 1, apparatus not exercised — committed by the operator at 88dff77. 27-05: `admit` once; the four attack legs refused on the record before and after the commit (8 byte-identical stderr captures, exit 1); RELRN-01 ticked, RELRN-02..05 left unticked as the named limitation (D-05); suite 2867 passed / 4 skipped / 0 failed (2871 collected) on the fully-tracked tree; the apparatus never ran on MPS; zero `gsd-sdk` mutation handlers. Earlier: waves 1-3 of 4 complete: 27-01 pre-registration (916ad4d, one commit; the gate reads MOOT on the committed frontier), 27-02 `on_draw` capture point (58ee800, 835b1d0), 27-03 driver (7e3d436, c4ba1c6), 27-04 D-22 fix + CPU wiring proof (c054d8b, 6d6ffb3); post-wave-3 suite 2867 passed / 4 skipped / 0 failed (2871 collected) |
| 28. Report, the Published Null, and Milestone Close | v4.0 | 7/7 | Complete | 2026-09-22 — verification `passed` (28-VERIFICATION.md: 4/4 ROADMAP SC, 35/35 plan must-haves, one recorded override for SC3's sha256 clause per D-27; every reading re-measured against HEAD 9dcf1fb); final suite on the closed tree 2914 passed / 4 skipped / 0 failed (25:12); code review 26c8ec0 — 0 critical / 4 warning / 4 info, advisory; phase checkbox ticked by hand, zero gsd-sdk mutation handlers. Earlier: v4.0 published as measured: null-at-both-capacities (docs/REPORT.md, publishing commit 3b63b7d), README glance bullets rendered, ledger results/phase28_ledger.json with the measured row counts (69 rows; v3.0 tech-debt 16 / stale-stamp 6 by len()), CI run 35770563251 green on origin/main at head d2dcbe2 (contains 3b63b7d; the developer pushed twice — the first run 35719377808 at 6896c31 failed on 4 tests, three causes measured and fixed at d2dcbe2: tags v2.0/v3.0 never pushed, a third occurrence of the refused correction helper's name in this row (tests/test_phase25_correction.py pins it by content), one Phase-27 host-gated skip on ubuntu; Claude never ran git push, D-38); close.ci_run recorded, rows byte-unchanged; the frozen block's ledger provenance row sized the whole file, so filling close moved 49057 → 49297 — fixed in the renderer at 89aae4f (size column excludes close as the digest does; render still byte-identical to 3b63b7d, no addendum needed); RPT-01/RPT-03 ticked by hand; zero gsd-sdk mutation handlers. Earlier: wave 5 of 6 complete: 28-06 PUBLISHED at 3b63b7d (SUMMARY 616ee8b) — docs/REPORT.md +283/-0 between PHASE28-REPORT sentinels, README.md +19/-0 glance bullets, byte-identity guards RED (4 failed, sentinels absent) → GREEN, normalized register widened 2 → 5, TD-XC-README-GLANCE → FIXED; renderer deviation fixed pre-publish (adv_points column carried condition (a)'s 0.0000% sentence, tripping STAT-02 — now the (c) reasons or the refusal); developer read and approved the rendered blocks (2026-09-21); D-20 freeze from 3b63b7d (write never runs again; append_addendum is the only route); check exit 0; post-wave suite 2913 passed / 4 skipped / 0 failed (25:11). Earlier: wave 4 of 6 complete: 28-05 (13ce840 20 guard tests, 236018e SUMMARY): template numeral scan (natural RED with a planted 32), obligation resolution by len() from the record (15 expansions per wildcard; the sigma=0 control verdict null is the named degenerate case), constants vs pins, digests, confound, lead, D-04/D-05 expectation ancestry (RED via merge-base --is-ancestor); post-wave suite 2905 passed / 4 skipped / 0 failed (24:42). Earlier: wave 3 of 6 complete: 28-04 (1a89294 renderer + two templates, 50ef5c9 SUMMARY): scripts/phase28_report.py stdlib-only, seven records loaded as bytes with sha256, every template numeral a binding (meta/frontier/canary/admission/sigma_zero/cost/control_floor/const/derived/table/quote/slice/path/ledger prefixes; missing binding raises), install between sentinel comments, check re-renders and diffs; derived.sigma_for_eps4 proves epsilon_for before printing 15.289937507119; two renders byte-identical; post-wave suite 2885 passed / 4 skipped / 0 failed (24:54). Earlier: wave 2 of 6 complete: 28-03 (841e7df ledger + 15 tests + todo resolved, 31d1538 SUMMARY): results/phase28_ledger.json — 69 rows (v3.0 27 / v4.0 42), NAMED-LIMITATION 23 / ACCEPTED 18 / FORBIDDEN-BY-GUARD 10 / FIXED 8 / RE-DEFERRED 6 / CLOSED-EARLIER 4, close.ci_run null; developer read the 69 rows and approved with no change (2026-09-21); audit-open total 2 (the two D-35 verification gaps by design); post-wave suite 2885 passed / 4 skipped / 0 failed (25:38). Earlier: wave 1 of 6 complete: 28-01 (d45eaec, dd087f7, 5103fcb: D-25 tomllib dependency-equality test across v1.0/v2.0/v3.0/HEAD, D-26 sha-pin test rename, D-32 docstring fixes with natural-RED regression tests) and 28-02 (64e7162, 4b00c80, 5cc6a3c: three quick-task SUMMARYs renamed + stamped, debug session resolved, five Phase-19 plan artifact names repaired, 25-UAT complete, 24-UAT items 2-3 disposed and its stamp flipped to complete by developer ruling 2026-09-21 — UAT-24-STAMP; audit-open 9 → 3, uat_gaps 0); post-wave suite 2870 passed / 4 skipped / 0 failed (24:50); sequential on main, no worktrees, zero gsd-sdk mutation handlers |
