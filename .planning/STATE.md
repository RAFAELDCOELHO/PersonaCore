...
stopped_at: "Phase 25 CONDITION-(c) REOPENING CLOSED (2026-08-31). 25-CONTEXT.md now carries 51 decisions across SEVEN areas — Area 7 (D-45..D-51) added purely additively, 121 insertions / 0 deletions, D-35 original text left standing and superseded by D-51. WHY REOPENED: condition (c) of the three-condition gate had NO measured inputs in the 20-plan set — 7 of mitigation_point_verdict's 21 kwargs had zero producers, six of them condition (c), and no Phase 23 point record carries dialogue_ppl/retention_ppl/ppl_on/ppl_off. MEASURED THIS SESSION: condition (c) costs 87.4 s/point (dialogue ON+OFF 43.5 s + retention 43.9 s) = 1.07 h over 44, or 0.80 h with the OFF leg measured once — 0.7-1.0% of the 107-150 h budget — and the measurement path reproduces results/phase19_arm_erased.json EXACTLY (5.815445876712191 / 4.573349214207799 / 4.219759892336485, denominators 270203 and 1000285), so (c) arrives with a free bit-level reproduction check. DECISIONS: D-45 all 44 points, cost measured before deciding; D-46 GATE-05 teacher-forced NLL gates on the 8 LOCKED facts at both capacities (the early return fires BEFORE reasons=[], so without it the pre-registered null — zero extraction under high noise — is structurally unreachable), full taught set reported beside and never a verdict input; D-47 control_gap PER CAPACITY per D-03; D-48 both floors imported as-is with the recipe mismatch disclosed, magnitude CHECKED first — the dialogue floor moves only 1.65% of the band, immaterial as measured; D-49 THE RETENTION LEG IS BINDING AT THE ANCHOR and that is PRE-REGISTERED BEFORE ANY POINT RUNS (v3.0 taught adapter fails the cap by +0.190760, 4.219760 vs 4.029000, needing a 2.38x larger floor; phase19 recorded adapter_on_headroom -0.1907598923364855 and no plan surfaced it) so an empty or narrow frontier can never be re-argued afterwards as a mis-set floor; D-50 every point record carries the counterfactual retention reading under a v4.0-recipe floor from the sweep's own seed-to-seed spread, no new run — FRONT-02's strongest form applied to (c); D-51 D-35 superseded via sentinel + _prose.normalized (never _addendum), counting toward RPT-02. STILL OPEN from plan-check iteration 2: 3 blockers (B1-NEW 25-20 deletion budget unsatisfiable, NEW-2 25-18 asserts ZERO TOLERANCE on all 44 but two early returns fire first, NEW-3 25-07 Task 1 has no working verification) + 5 warnings. The 20 plans on disk do NOT yet cover D-45..D-51. Next: /gsd-plan-phase 25 to fold Area 7 into the plan set and close the 3 blockers + 5 warnings."
...
Plan: 20 plans / 51 tasks / 13 waves on disk (b8069ea) — they do NOT yet cover Area 7 (D-45..D-51).
