...
**Status:** fixed 2026-09-09 in `795edb3` — `needs_adapters` (6 tests, short-circuited on `(_ROOT / "checkpoints").is_dir()`) and `needs_plutil` (1 test) in the recall file; the venue file's ubuntu pins continued as a dated addition to `52 + 3 + 7 = 62 / 62` with the M3 39 / 4 untouched, and the words "confirmed by CI" withdrawn — 62 is DERIVED and awaits the next CI run. Both guard expressions proved False against an empty temp root and an empty PATH without touching `checkpoints/`.
...
**Status:** fixed 2026-09-09 in `cb96d8c` — `build(*, overwrite=False)` and `emit(out_path=RECORD, *, overwrite=False)` refuse an existing artifact as their FIRST statement, naming `phase25_record.RERUN_ROUTE`'s sanctioned route, with `--force` on both command lines as the deliberate escape hatch. A plain `_prove` rather than `phase21_unit_record.refuse_existing_artifacts`, whose `teach_persona` import would put torch in `sys.modules` and break the recall driver's CPU-safe-at-import property. One test per writer asserts the refusal fires against the COMMITTED file and leaves its bytes unchanged; neither writer was run. The frontier-digest assertion the Fix also suggests is NOT added (the review confirms the digests currently match; the write-once refusal removes the path that would break them).
...
**Status:** fixed 2026-09-09 in `cb96d8c` — the `except SystemExit` now records only a message matching `COVERAGE_FLOOR_REFUSAL_MARKERS` (two substrings quoted from `phase20_gate_coverage`'s recall-floor `_prove`); anything else propagates. The recorded `adv_n64` refusal is byte-identical (`str(refusal)` still stored verbatim) and the live-reproduction test stays green; a new test asserts the recorded refusal matches the markers, that the markers are still present in `phase20_gate_coverage.py`, and that a structural message does not match.
...
**Status:** deferred 2026-09-09 in `83f74b4` as **D-25-REVIEW-WR03** (`deferred-items.md`) — the string is published verbatim in the write-once `results/phase25_frontier.json`, so correcting it is a dated continuation in Phase 26, not a re-emit. Nothing computed is wrong: `_lot_from_train_config` applies the correct rule per arm.
...
**Status:** deferred 2026-09-09 in `83f74b4` as **D-25-REVIEW-WR04** (`deferred-items.md`) — every literal matches today, `results/phase25_promotion.json` is pinned by the frontier and must not be re-emitted, and WR-01's write-once refusal removes the re-run path that would have made the prose stale.
...
**Status:** the ten Info findings are OUT OF SCOPE for this fix pass (`--fix` scope: Critical + Warning). None is addressed; each stands as written.
...
CR-01 fixed (`795edb3`), WR-01 + WR-02 fixed (`cb96d8c`), WR-03 + WR-04 deferred (`83f74b4`). `make lint`: All checks passed, 275 files already formatted.

    $ .venv/bin/python -m pytest tests/test_phase25_recall.py tests/test_phase25_promotion.py tests/test_phase25_venue.py -q
    47 passed, 3 skipped in 873.61s (0:14:33)

The venue file's two inner suites ran inside that number, so the two pinned skip counts (39 sweep-active / 4 flag-unset on this M3) were re-measured, not asserted from cache. The five ancestry-guarded modules, `pyproject.toml` and the three pinned artifacts (`phase25_frontier.json`, `phase25_promotion.json`, `phase25_recall.json`) are byte-unchanged.
