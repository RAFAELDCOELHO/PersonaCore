...
- 🚧 **v3.0 Adversarial Privacy Audit and Selective Memory Erasure** — Phases 16-19 (**all 4 phases complete 2026-08-19**; milestone close is a separate act and has not been taken) *(Phase 19 Selective Erasure entered 2026-08-17 — the pre-registered gate returned True on Phase 18's measured numbers — and closed 2026-08-19 with verdict `FAILURE` and ship decision `DO NOT SHIP`)*
...
- [x] **Phase 19: Selective Memory Erasure** - Erase one taught fact from the weights under the rule committed at `23a830c`; blind-calibrated target floor, per-fact non-target preservation, capability caps, representational consistency descriptive-only (**completed 2026-08-19** — 16 plans in 14 waves, ALL EXECUTED. Verdict `FAILURE`, published unsoftened; ship decision **`DO NOT SHIP`**, withholding only the claim that the verdict is mechanically reproducible by the pin alone and withdrawing no measurement. **THE TARGET FACT HAS BEEN ERASED** at 19-12, k = 78 of 288 rank-1 components, and the human took **D8 branch = the cliff, with the rank-vs-NLL disagreement as CO-HEADLINE**: selective erasure is NOT selective at 331,776 parameters — (a) clears perfectly at 0/27 exactly on the floor while ALL SEVEN gated non-targets fail, four at total generation loss, and 77.6% of the dialogue adaptation is destroyed. The rank instrument reports none of it: seven ruined facts still read rank 1 at ceiling exposure at every checkpoint. At 19-13 ERASE-02 was discharged by a RUN and the co-headline got its second, harder data point: the rank instrument reports M1 and M2 as BIT-IDENTICAL across all eight slots while M2 keeps five of seven bystanders at delta exactly 0.0. **At 19-14 a THIRD instrument was read, and it is DESCRIPTIVE by construction and reaches no gate:** the per-cell ΔW cosine separates the two regions exactly — all 14 preserved cells at 1.0 to fp64 round-off against 22 ablated cells spanning 0.4764 … 0.9590 with one undefined, landing on the single cell whose all 8 rank-1 components were zeroed — and it returns two different shapes for M1 and M2 where the rank instrument returned bit-identical readings. Reported as numbers only; three committed AST/artifact scans now enforce that the producers, the consumers in both files, and the records themselves hold no threshold, no verdict and no p-value. 19-14 also MEASURED a live blocker for 19-15 by driving `report` rather than inspecting it: the pinned `report` subcommand SystemExits on the committed arm records at 19-09's defect C, so the verdict must be rendered by calling `render_report` directly with pooled rows)
...
- [x] 19-12-PLAN.md — CHECKPOINT: erase the target by M1, record the collateral curve, score at A2/K=48
- [x] 19-13-PLAN.md — ERASE-02: the retrain-without reference arm, run rather than explained
- [x] 19-14-PLAN.md — the DESCRIPTIVE representational read, with the not-gated guard re-run against it
- [x] 19-15-PLAN.md — the single `erasure_succeeded()` call and the report; (c) runs literally
- [x] 19-16-PLAN.md — CHECKPOINT: the dated (c) diagnosis beside the verdict, the ship decision, phase close — **SHIP DECISION RECORDED 2026-08-19: `DO NOT SHIP`**, withholding exactly ONE claim (that the verdict is mechanically reproducible by the pin alone) and withdrawing no measurement
...
**Totals:** 19 phases complete, 122 plans (29 v1.0 + 39 v2.0 + 54 v3.0 — counted off the table above, not carried forward), 2 milestones shipped; **all 4 v3.0 phases COMPLETE — Phase 19 CLOSED 2026-08-19 at 16 of 16 plans.** The pre-registration was complete, human-reviewed and CLOSED at 19-07; at 19-08 the ordering stopped being a claim, and at 19-09 the ancestry guard checks 15 pin commits against 15 tracked `results/phase19_*` artifacts — checked = 225, non-vacuous and green. At 19-11 the three measured constants were LOCKED in `scripts/phase19_floor.py` and a human APPROVED all three severities knowingly, including that `TARGET_FLOOR` = `0.09107873950450847` IS `wilson_upper_bound(0, 27)` exactly, so (a) clears ONLY on a perfect erasure. **AT 19-12 THE TARGET WAS ERASED AND THE PHASE HAS ITS RESULT, TAKEN ON THE D8 BRANCH COMMITTED AT 19-05 BEFORE THE NUMBER EXISTED — CLAUSE 1, THE CLIFF, UNSOFTENED: `selective erasure is not selective at 331,776 parameters`. M1 stopped at k = 78 of 288, dispersed across all six layers (18/12/12/17/10/9) and all six projections (`fc_in` 35, `fc_out` 17, `c_proj` 13, `v_proj` 10, `k_proj` 2, `q_proj` 1) with no localisation anywhere. (a) clears PERFECTLY and exactly on its boundary — 0 successes over the pooled 27 (0/13 held-out, 0/14 taught, 1,296 draws), Wilson upper bound EQUAL to `TARGET_FLOOR`, headroom 0.0. (b) FAILS ON ALL SEVEN gated non-targets, deltas 0.3704 … 1.0 with FOUR at total generation loss. (c)'s dialogue leg still fails (+0.2674 over cap) and its retention leg clears (−0.3581) only because the personalization is gone. THE CO-HEADLINE, PUBLISHED AT EQUAL WEIGHT: the rank/exposure instrument reports the OPPOSITE of the generation instrument on the same weights — all seven ruined non-targets hold rank 1 with exposure bits at ceiling at every one of the eight checkpoints, while the dialogue ON/OFF gap falls 1.2421 → 0.2778 and 77.6% of the adaptation is destroyed. This carries explicit RETROACTIVE weight on Phase 18's rank-based readings: the same instrument may be BLIND to exactly the kind of damage the generation reading reveals, so any Phase 18 conclusion resting on rank or exposure bits ALONE must be re-read with that in view.** AT 19-13 ERASE-02 WAS DISCHARGED BY A RUN AND THE CO-HEADLINE GOT ITS SECOND, HARDER DATA POINT: the M2 retrain-without arm trained in `wall=81s` and scored at A2/K=48 over 10,368 draws with `assert_phase18_parity` ENFORCED. The omitted fact reads **0/27** pooled over 1,296 draws, so M2's own falsification condition did NOT fire and 19-12's target number survives. **Five of the seven bystanders move by EXACTLY 0.0** from the taught adapter and the two that move (house_number −0.2593, hometown −0.1111) are both below the 0.2963 margin — against M1's seven failures spanning 0.3704 … 1.0 with four at total loss. Soft tier taught 54/54 questions → M1 1/54 → **M2 53/54**. M2's capability legs land on the TAUGHT adapter and both still FAIL (dialogue +1.4242, retention +0.1882), which is the direct evidence that M1's retention leg clearing was the personalization being gone rather than the erasure succeeding. **AND THE RANK INSTRUMENT REPORTS M1 AND M2 AS BIT-IDENTICAL ACROSS ALL EIGHT SLOTS — every rank and every `exposure_bits` value equal — so it cannot distinguish an adapter whose bystanders generate 0/27 from one whose same bystanders generate 27/27.** Reported as a reference point only; `ERASURE_GOAL_FRAMING` forbids any statement of indistinguishability, and that constraint is a required field of the record rather than report prose.) **AT 19-15 THE COMMITTED `erasure_succeeded` WAS CALLED EXACTLY ONCE AND RETURNED `FAILURE`** — (a) clearing exactly on its boundary, (b) failing on all seven gated non-targets, (c) failing on the dialogue leg — published unsoftened. **AT 19-16 THE PHASE CLOSED.** A dated 2026-08-19 continuation diagnoses (c)'s root cause BESIDE the verdict without amending `23a830c`: the literal (c) caps against the adapter-OFF `V20_MASKED_DIALOGUE_VAL_PPL` 4.5733 while post-erasure capability preservation would have to be read against the adapter-PRESENT 5.8154 Phase 14 already published as a limitation, so a PERFECT erasure would still return FAILURE on (c) alone. Both readings ship side by side with the gate's row labelled. **AND THE SHIP DECISION IS RECORDED: `DO NOT SHIP`.** It withholds EXACTLY ONE claim — that the verdict is MECHANICALLY REPRODUCIBLE BY THE PIN ALONE — and withdraws no measurement: the verdict, k = 78 of 288, the 77.6370113463966% destruction, the rank-vs-NLL CO-HEADLINE with its Phase 18 scope limit, the M1/M2 comparison and all four dated defect corrections all STAND. The verdict was reached on a hand-driven path through the UNPINNED `phase19_run.py`, and the pin's own `_cmd_report` cannot reproduce it four independent ways (C SystemExits, the fourth raises `TypeError`, A short-circuits to INCONCLUSIVE, B reads the superseded 0.2 floor). **This is a FINAL verdict on reproducibility, not a blocker and not a TODO — the pin must NOT be repaired to flip it, because editing a CLOSED pin after the numbers exist voids the pre-registration ordering and every number with it.**
