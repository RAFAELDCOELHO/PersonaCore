...
RESOLVED 2026-09-09 — run 34403612853 (ubuntu-latest, the first CI run ever to execute waves 10-13) reported `8 failed, 2682 passed, 62 skipped`: **the derived 62 was exactly right**, and the eight failures were real defects invisible on the publication host — seven were the epsilon ladder's two glibc twins (4 ULP at sigma 8.0 and 50.0) reaching three comparison sites that had never run off Apple Silicon, which made `phase25_points` refuse to resolve the 44 plans on x86 at all and would have killed the documented Kaggle P100 fallback; one was a LaunchAgent plist's absolute path asserted against the checkout root. Fixed in `ce2a151` following the repo's OLDER decision (named twins under exact `==`, never a tolerance — see `scripts/phase25_epsilon.py::LADDER_PLATFORM_TWINS`) rather than the relative tolerance first considered; no published epsilon changed. Re-run `34406246073`: **`2691 passed, 62 skipped, 9 warnings in 1245.48s`, green**. The venue pin is now MEASURED with both run ids beside it, and the finding is recorded in `results/phase25_operational_note.md` §13.9.
