...
| Phase-23 pre-registration (blind floor reduction, blind seed-count rule, the D-04 halt verdict, the D-06 withdrawal rule, and the canonical `results/phase23_*` path register) | `scripts/phase23_prereg.py` — **new, this phase, EDIT-ONCE** | — | **The placement is forced, not preferred.** It cannot live in `scripts/mitigation_budget.py`: that file is literal-only and zero-import by §R2.3, so it cannot hold `choose_n_seeds` at all — and it is not edit-once, so it cannot carry a pre-registration. It cannot live in `scripts/phase23_run.py`: four later plans re-edit that driver, so `git log -1` on it returns its most recent commit and no ancestry check could bind a rule there to the measurement it decides. This module is edit-once from 23-04's first artifact, closed by `test_the_prereg_rule_precedes_every_phase23_result`. |
