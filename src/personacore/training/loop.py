    ...
    if any(value is not None for value in _fact.values()):
        ...
        if max(1, train_config.grad_accum_steps) != n_facts:
            raise ValueError(
                ...
                "'grad_accum_steps' appears 13 times in scripts/teach_persona.py and exactly 1 "
                "of them is CODE. Before plan 22-10 wired the production caller the code count "
                "was 0 -- every hit was a docstring, a comment or an error string -- so the "
                "TrainConfig(...) built there inherited the default 1 "
                "(TrainConfig.grad_accum_steps in config.py) and the aligned three-bin corpus "
                "would have trained at a LOT SIZE OF ONE with SC2 true only in prose. That one "
                "code hit is a DICT KEY in a splat ({'grad_accum_steps': stats['n_facts']} on "
                "the DP arms only), which is why counting it needs ast.Constant and not only "
                "ast.keyword/Attribute/Name. Under dp_fn, "
