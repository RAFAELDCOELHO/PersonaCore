    ...
    if any(value is not None for value in _fact.values()):
        ...
        if max(1, train_config.grad_accum_steps) != n_facts:
            raise ValueError(
                ...
                "'grad_accum_steps' appears 14 times in scripts/teach_persona.py and exactly 1 "
                "of them is CODE -- the dict(grad_accum_steps=stats['n_facts']) splat plan 22-10 "
                "wires on the DP arms only. Before that plan the code count was 0: every hit was "
                "a docstring, a comment or an error string, so the "
                ...
                "would have trained at a LOT SIZE OF ONE with SC2 true only in prose. Under "
                "dp_fn, "
